	...
	*/
	init: function() {
		...
		*/
		...
		this.specs.smallest.startY = Math.ceil( this.specs.tile.grossTextTop + this.specs.smallest.fontSize * 0.83 );
			availableHeight = this.specs.tile.height - this.specs.label.lineHeight - this.specs.tile.margin - this.specs.smallest.startY;
		this.specs.smallest.maxLines = 1 + Math.floor( availableHeight / this.specs.smallest.lineHeight );
		...
		* I could have used the equivalent jQuery('#add_to_dom').on('click' ... but
		* I didn't. I don't remember why.
		*/
		document.getElementById('add_to_dom').addEventListener('click', function() {
		...
		/**
		* Slightly fancier, download the canvas image as a PNG.
		* I've used an anchor element because I can simply add a 'download' attribute (= filename)
		* and the browser will take care of the rest...	
		* Quiz yourself: what is the value of 'this' in the following click handler?
		* (Hint: I've used 'self' to reference the PPTile object.)
		*/
		document.getElementById('download_image').addEventListener('click', function() {
			var dataURL = self.cvs.toDataURL( 'image/png', 1 );
			this.href = dataURL;
			this.download = 'CanvasImage.png';
		});
	...
	redrawMessage: function( msg ) {
		...
		if ( lob.lines.length ) {
			...
			this.ctx.fillText(
				...
				this.intro.split('').join( String.fromCharCode(8201) ),					// the text
